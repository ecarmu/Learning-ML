...
def test_tower(target):
    ...
    for layer in target.layers:
        ...
        computed_shape = layer.compute_output_shape((None, 14))
        assert computed_shape == expected[i][1], \
            f"Wrong number of units in layer {i}. Expected {expected[i][1]} but got {computed_shape}"
